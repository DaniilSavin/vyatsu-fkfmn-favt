...
dll = CDLL(r'C:\Users\Мария\OneDrive\Рабочий стол\asd\pythonlab4\libtest.so')
...
class Window:
    ...
    def __init__(self, width, height, title, resizable=(False, False), icon=None):
        ...
        self.Button_5 = Button(self.root, text="Количество чисел", command=self.CountNumbers).pack(side=LEFT, pady=0, padx=5)
    ...
    def CountWords(self):
        self.TimeFileUse()
        s = module.CountWords('TimeFile.txt')
    ...
    def CountCharacters(self):
        self.TimeFileUse()
        s = module.CountCharacters('TimeFile.txt')
    ...
    def CountLines(self):
        self.TimeFileUse()
        s = module.CountLines('TimeFile.txt')
    ...
    def CountNumbers(self):
        self.TimeFileUse()
        dll.CountNumbers.restype = c_int
        dll.CountNumbers.argtypes = [c_char_p]
        dll.CountNumbers(b'TimeFile.txt')
        s = str(c_int.in_dll(dll,"counter"))
        s = s[7:-1] 
        messagebox.showinfo('Статистика файла', 'Количество чисел: ' + s)
        os.remove('TimeFile.txt')
