...
void tests(string path, string pathout)
{
    ...
    for (int i = 0; i < First_Coeffs_Size; i++)
    {
        ...
        {
            auto n = max(First_Coeffs[i].size(), Second_Coeffs[j].size());

            extend_vec(First_Coeffs[i], n);
            extend_vec(Second_Coeffs[j], n);

            Broker = karatsuba_mul(First_Coeffs[i], Second_Coeffs[j]);
    ...
    for (long long i = 0; i < Final_Size; i++)
    {
        ofs << "x^" << i << " * ";
        if (Final_Signs[i] == false)
            ofs << "-";
        for (long long j = Final[i].size() - 1; j >= 0; j--)
        {
            ofs << Final[i][j];
        }
        ofs << endl;
